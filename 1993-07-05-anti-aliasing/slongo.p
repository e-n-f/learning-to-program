...
{drawChar('&');}
	...
	procedure realDrawString (s: str255);
	...
	begin
		...
		if txSize > 36 then
			...
			begin
				...
				playRect.topLeft.v := (txSize * mul) - (ascent * mul);
				playRect.botRight.h := BigCw + 4 + 8;
				playRect.botRight.v := (txSize * mul) + (descent * mul);
...
begin
	setRect(theRect, 0, 419, screenbits.bounds.right, screenbits.bounds.bottom);
